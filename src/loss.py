...
def loss_generator(image, text, text_length, D, G, lambda_1, params):
    ...
    uncond, cond = D(fake, negative_text, negative_text_length, negative=False)
...
def loss_real_discriminator(image, text, text_length, D, G, lambda_1, params):
    uncond, cond, cond_n = D(image, text, text_length, negative=True)
...
def loss_synthetic_discriminator(image, text, text_length, D, G, params):
    ...
    fake, _ = G(image, (negative_text, negative_text_length))
    uncond, _ = D(fake.detach(), negative_text, negative_text_length, negative=False)
